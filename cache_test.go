...
func TestInvalidCapacity(t *testing.T) {
	assert.Panics(t, func() {
		New(Config[string]{Capacity: 0})
	})
}

func TestInvalidMaxAge(t *testing.T) {
	assert.Panics(t, func() {
		New(Config[string]{Capacity: 1, MaxAge: -1 * time.Hour})
	})
}

func TestInvalidMinAge(t *testing.T) {
	assert.Panics(t, func() {
		New(Config[string]{Capacity: 1, MinAge: -1 * time.Hour})
	})
...
func TestBasicSetGet(t *testing.T) {
	cache := New(Config[int]{Capacity: 2})
	cache.Set("foo", 1)
	cache.Set("bar", 2)
...
func TestHas(t *testing.T) {
	cache := New(Config[string]{Capacity: 1, MaxAge: time.Millisecond})
	cache.Set("foo", "bar")
	<-time.After(time.Millisecond * 2)
...
func TestLen(t *testing.T) {
	cache := New(Config[int]{Capacity: 10})
	for i := 0; i <= 9; i++ {
		evict := cache.Set(i, i)
		assert.False(t, evict)
	}
...
func TestSetMinAge(t *testing.T) {
	cache := New(Config[int]{Capacity: 10, MaxAge: time.Hour})
